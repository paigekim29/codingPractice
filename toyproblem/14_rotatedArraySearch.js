...
const rotatedArraySearch = function (rotated, target) {
  const half = Math.floor((rotated.length -1)/2)
  const recursion = function(arr, target, start, end){
    ...
    else return recursion(arr, target, start, half -1)
  }
  if(rotated[half] === target) return half
  if(target > 0 && target < rotated[rotated.length - 1]) return recursion(rotated, target, half + 1, rotated.length -1)
  return recursion(rotated, target, 0, half -1)
};
